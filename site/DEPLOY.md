...
Na pasta **Empreendimento Jardini** (a que contém `site/` e as pastas 1 a 7):
...
3. **Root Directory**: escolha a pasta que tem **ao mesmo tempo** a pasta `site/` e as pastas `1. Apresentação Clientes`, `2. Plantas apartamentos`, etc.  
   - Se o repositório for só o conteúdo de “Empreendimento Jardini”, deixe **Root Directory** em branco.
4. **Build**: em projetos estáticos a Vercel costuma detectar que não há build. Se pedir “Output Directory”, use a pasta que contém o `index.html` (por exemplo `site`), e garanta que as pastas 1–7 e a foto da corretora estejam no mesmo nível que `site/` (para os `../` funcionarem).
...
4. Em **Branch** selecione `main` (ou `master`) e a pasta **/ (root)**. Se o seu `index.html` estiver dentro de `site/`, use a branch e a pasta **/ (root)** e garanta que a raiz do repo tenha o `index.html` ou configure **Custom source** para a pasta `site` (o GitHub Pages pode servir só uma pasta; para manter `site/` e as pastas 1–7 no mesmo nível, a estrutura do repo precisa permitir que o Pages sirva a pasta certa — por exemplo, colocar o conteúdo de `site` na raiz do repo junto das pastas de mídia).
5. Salve. Em alguns minutos o site estará em `https://SEU_USUARIO.github.io/jardine-residence/`.

**Importante para GitHub Pages:** o site usa `../` para pastas irmãs. O Pages serve apenas uma pasta (a raiz ou uma subpasta). Para funcionar, o repositório precisa ter na **raiz** o `index.html`, `index-es.html`, `estilos.css` e as pastas `1. Apresentação Clientes`, `2. Plantas apartamentos`, etc. (ou seja, mesma estrutura da Opção A do deploy). Se hoje tudo está em `site/` com `../` apontando para fora, você pode: (a) mover os HTML/CSS para a raiz e trocar `../` por `./`, ou (b) usar Vercel, que permite configurar a pasta de output.
