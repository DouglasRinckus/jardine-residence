...
| Site   | URL de acesso |
|--------|-------------------------------|
| Jardine | `https://seu-usuario.github.io/jardine-residence/jardine/` (ou seu domínio customizado + `/jardine/`) |
...
- **Como:** a raiz do repo já tem `index.html` que redireciona para `/jardine/`. A página do Nilo fica em `Nilo/index.html`. Depois do push, acesse `/Nilo/` no mesmo domínio do Pages.
...
- **Mesmo domínio, URLs diferentes?** Sim: use **Opção 1** (um repo, pastas `jardine/` e `Nilo/`).
- **Domínios diferentes (ex.: jardine.com e nilo.com)?** Use **Opção 2** (dois repositórios e um domínio customizado por repo).

A página do Nilo foi feita para funcionar na **Opção 1** (acesso por `.../jardine/` (Jardine) e `.../Nilo/` (Nilo) no mesmo repo). Se depois você criar um repo só para o Nilo, basta ajustar os caminhos dos assets (CSS, imagens) para o novo contexto.
